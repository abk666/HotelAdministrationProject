...
public class CheckInBookingController implements Initializable{
    ...
    void processRoomType(ActionEvent event) throws SQLException {
	  ...
	  ObservableList<Integer>roomNo=FXCollections.observableArrayList();
			...
			ObservableList<Room> roomList = roomDataUtils.getAllRoom("select * from room where roomType = '"+cobRoomType.getValue()+"' and roomStatus ='Booked';");
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		try {
			ObservableList<Room> roomList = roomDataUtils.getAllRoom("select * from room where roomStatus ='Booked';");
			 for(Room room:roomList) {
		    	  roomNo.add(room.getRoomNumber());
		      }
		} catch (SQLException e) {
		...
		cobRoomNo.setValue(booking.getRoomNo());
		tfGuestRoomPrice.setText(booking.getRoomPrice().toString());
		
		dpGuestCheckInDate.setValue(LocalDate.parse(booking.getCheckInDate().toString()));
		
		dpGuestCheckOutDate.setValue(LocalDate.parse(booking.getCheckOutDate().toString()));
